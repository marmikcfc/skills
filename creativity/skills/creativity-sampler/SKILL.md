...
description: Generate probability-weighted alternative options that challenge default thinking, fight anchoring/typicality bias, and expose hidden assumptions behind the "obvious" choice. Use at a decision point when you want options beyond the first answer — picking a technology/architecture/approach/design/strategy, "what are my alternatives", "give me other options", "what am I not considering", or whenever you catch yourself defaulting to one obvious answer. For decision-point analysis, NOT full open-ended design exploration. Also triggers on "대안", "alternatives", "옵션 뽑아", "options", "어떤 방법이", "아이디어", "다른 방법", "선택지".
...
- Factual questions with one correct answer (use a research or verification approach instead)
...
This skill is one *generator* in the `creativity` pack. It pairs naturally with:

- **creative-generation:** This sampler is one of several Phase-3 generators in the `creative-generation` orchestrator. Use it standalone for a quick decision; use the orchestrator when you want a wider mechanistic spread (analogy, blending, abductive gap-finding) on top of probability-zone sampling.
- **constraint-based-creativity:** If the obvious options all feel samey, impose a constraint first, then sample — constraints sharpen what "unconventional" means.
- **forming-opinions (thinking-models):** Feed the chosen option into the opinion stress-test to pressure-test the call before committing (credence + falsifier).
